...
| 3 | Typed layer: vocabulary (`Phase`/`Side`/`Dofs` generating the set-name grammar), typed registry entries (pointer + `std::function`), OPT criteria-expression API (values by tree evaluation, criteria gradients by reverse-mode, `IQI_types`/`constraint_types` derived automatically) | `cl_Input_Deck_Vocabulary.hpp`, `cl_Input_Deck_Expressions.hpp` in MRS/IOS — header-only, so the planned `projects/DCK/` package was folded into IOS next to `Input_Deck`; trivial to move later if DCK materializes |
